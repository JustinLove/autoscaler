...
module Autoscaler
  module Sidekiq
    ...
    class MonitorMiddlewareAdapter
      ...
      # @param [Strategy,Numeric] timeout strategy object that determines target workers, or a timeout to be passed to {DelayedShutdown}+{ZeroOneScalingStrategy}
      ...
      def strategy(timeout)
        if timeout.respond_to?(:call)
          ...
          DelayedShutdown.new(ZeroOneScalingStrategy.new, timeout)
        end
      end
    end
  end
